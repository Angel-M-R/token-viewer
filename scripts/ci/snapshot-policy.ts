...
const CANONICAL_SNAPSHOT_PATH =
  /^snapshots\/(angel-mac|old-mac|mac-m5)\/\d{4}\/\d{2}\/\d{4}-\d{2}-\d{2}\.json$/;
...
export function assertSingleMachineSnapshotPublication(paths: readonly string[]): void {
  ...
  if (machines.has("old-mac")) {
    throw new SnapshotPolicyError("Data publication cannot publish from retired machine folder: old-mac");
  }

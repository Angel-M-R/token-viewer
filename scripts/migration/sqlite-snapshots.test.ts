...
describe("one-time SQLite snapshot migration", () => {
  it.each(["angel-mac", "aon-mac"] as const)("imports %s independently without private source fields", async (machine) => {
  ...
  it("keeps imported historical aon-mac snapshots byte-for-byte immutable", async () => {
    const fixture = await createFixture();
    addUsage(fixture.databasePath, "aon-mac", { hash: "historical", date: "2025-09-12" });
    await writeDailySnapshotAtomic(fixture.repositoryRoot, emptySnapshot("aon-mac", "2025-09-12"));
    const path = snapshotPath(fixture.repositoryRoot, "aon-mac", "2025-09-12");
  ...
  it("never assigns retired aon-mac SQLite history to aon-mac-m5", async () => {
    const fixture = await createFixture();
    addUsage(fixture.databasePath, "aon-mac", { hash: "retired", date: "2025-09-12" });
  ...
  it("deduplicates source records inside SQLite without persisting their hashes", async () => {
    const fixture = await createFixture();
    addUsage(fixture.databasePath, "aon-mac", { hash: "duplicate", date: "2025-09-16" });
    addUsage(fixture.databasePath, "aon-mac", { hash: "duplicate", date: "2025-09-16" });
...
async function createFixture(): Promise<{ root: string; repositoryRoot: string; databasePath: string }> {
  ...
  database.exec(`
    ...
    INSERT INTO machines (id, name) VALUES (1, 'angel-mac'), (2, 'aon-mac');
  `);
  database.close();
...
function addQuota(databasePath: string, machine: "angel-mac" | "aon-mac", date: string, percentUsed: number): void {
  const database = new DatabaseSync(databasePath);
  database.prepare(`
...
function emptySnapshot(machine: "angel-mac" | "aon-mac", date: string) {
  return {

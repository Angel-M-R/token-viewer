...
The daily publisher is limited to the active identities `angel-mac` and `aon-mac-m5`. Each active Mac must use its own dedicated checkout on `master`, and that checkout must never be used for development. Do not install or activate either job until the migration branch has passed normal review and has been integrated. The retired `aon-mac` identity remains valid only for reading its historical snapshots.
...
Use these values on the Aon M5 Mac:

```sh
export CHECKOUT="$HOME/TokenViewer-ops/aon-mac-m5"
export EXPECTED_REMOTE="git@github.com:OWNER/tokenViewer.git"
export LABEL="com.tokenviewer.collector.aon-mac-m5"
```
...
```sh
node "$CHECKOUT/ops/macos/install-launchd.mjs" \
  --machine aon-mac-m5 --checkout "$CHECKOUT" --hour 9 --minute 0 --dry-run
node "$CHECKOUT/ops/macos/install-launchd.mjs" \
  --machine aon-mac-m5 --checkout "$CHECKOUT" --hour 9 --minute 0
```
...
pnpm --dir "$CHECKOUT" --filter collector exec tsx src/cli.ts status
launchctl print "gui/$(id -u)/$LABEL"
tail -n 200 "$HOME/Library/Logs/TokenViewer/aon-mac-m5.out.log"
tail -n 200 "$HOME/Library/Logs/TokenViewer/aon-mac-m5.err.log"
```
...
`aon-mac` is retired and read-only. Its already imported aggregate snapshots under `snapshots/aon-mac/` remain valid for validation and dashboard history, but this identity has no collector generation, Git publication, dedicated operational checkout, manual publisher command, or `launchd` installation path. Both the publisher and installer reject `aon-mac`; do not repurpose its historical folder for `aon-mac-m5`.

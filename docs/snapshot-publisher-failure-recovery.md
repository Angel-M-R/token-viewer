...
Use this procedure only in the affected active machine's dedicated `master` checkout. Set `CHECKOUT`, `MACHINE`, and `LABEL` to that checkout, either `angel-mac` or `aon-mac-m5`, and its installed launchd label. Never use `reset --hard`, `clean`, force-push, or edits to another machine's folder. Retired `aon-mac` has no publisher checkout or `launchd` job; preserve its historical snapshots as read-only data.

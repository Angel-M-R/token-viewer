...
describe.sequential("launchd installer", () => {
  ...
  it("rejects retired aon-mac before creating or loading a plist", async () => {
    ...
    await writeFile(
      configPath,
      `${JSON.stringify({ machineName: "aon-mac", checkoutPath: checkout, expectedRemoteUrl: remote })}\n`,
      "utf8",
    );
    ...
    expect(result.stderr).toContain("angel-mac or aon-mac-m5");
    expect(await exists(join(root, "Library", "LaunchAgents", "com.tokenviewer.collector.aon-mac.plist"))).toBe(false);

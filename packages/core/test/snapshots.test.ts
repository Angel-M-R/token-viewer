...
describe("snapshot contract", () => {
  ...
  it("separates active publisher identities from the historical snapshot identity", () => {
    expect(SNAPSHOT_MACHINES).toEqual(["angel-mac", "aon-mac", "aon-mac-m5"]);
    expect(ACTIVE_PUBLISHER_MACHINES).toEqual(["angel-mac", "aon-mac-m5"]);
    ...
    expect(activePublisherMachineSchema.safeParse("aon-mac").success).toBe(false);
  });
  ...
  it("rejects machine and date disagreement with the path", () => {
    ...
    (wrongMachine.value as { machine: string }).machine = "aon-mac";

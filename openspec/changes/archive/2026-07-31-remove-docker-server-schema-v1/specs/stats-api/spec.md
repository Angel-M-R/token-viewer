...
**Reason**: Los snapshots contienen agregados diarios y no registros individuales.
**Migration**: Usar las vistas agregadas existentes.

#### Scenario: Sin registros individuales
- **WHEN** el usuario navega por el dashboard
- **THEN** no existe una consulta ni vista de registros individuales

### Requirement: Liveness
**Reason**: No existe proceso servidor que comprobar.
**Migration**: Validar build y carga local de snapshots.

#### Scenario: Ejecución local
- **WHEN** se inicia el dashboard
- **THEN** no depende de un healthcheck HTTP

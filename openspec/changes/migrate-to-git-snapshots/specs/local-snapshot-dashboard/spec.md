...
La capa local SHALL aplicar los filtros actuales de máquina, agente, proveedor, modelo y rango temporal sobre las filas horarias de `angel-mac`, `aon-mac` y `aon-mac-m5`. Los valores disponibles de cada filtro MUST derivarse del conjunto cargado y las combinaciones de filtros MUST tener la misma semántica inclusiva que el dashboard actual. La identidad histórica `aon-mac` MUST permanecer consultable aunque no pueda publicar.
...
- **WHEN** el usuario filtra por `aon-mac`

...
TokenViewer SHALL reconocer exclusivamente `angel-mac`, `aon-mac` y `aon-mac-m5` como identidades de snapshot. `angel-mac` y `aon-mac-m5` MUST ser las únicas identidades publicadoras activas; `aon-mac` MUST ser histórica y de solo lectura. Una identidad activa configurada localmente MUST autorizar escritura únicamente en `snapshots/<machine>/`; no se creará un registro dinámico, token de máquina ni secreto compartido.

#### Scenario: Identidad publicadora permitida
- **WHEN** el collector se configura como `aon-mac-m5`
- **THEN** acepta la identidad y limita todas sus escrituras a `snapshots/aon-mac-m5/`

#### Scenario: Identidad histórica válida para lectura
- **WHEN** el validador o dashboard carga snapshots bajo `snapshots/aon-mac/`
...
- **WHEN** el collector, publicador o instalador de `launchd` recibe `aon-mac` como identidad operativa
...
**Migration**: Reemplazar el alta por configuración local de una identidad publicadora activa y credenciales Git personales externas; conservar `aon-mac` solo como identidad histórica de snapshots.

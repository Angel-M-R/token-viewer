...
El binario `tokenviewer-collector` SHALL exponer una operación que genere los snapshots pendientes de la máquina configurada, los valide y, cuando se solicite publicación, ejecute el flujo Git seguro. La operación MUST limitar la identidad publicadora a `angel-mac` o `mac-m5`, MUST rechazar `old-mac` como identidad operativa y MUST terminar con exit code distinto de 0 si generación, validación, commit o publicación fallan.
...
- **WHEN** se intenta generar o publicar con `machineName = "old-mac"`
...
El collector SHALL leer su configuración local respetando las rutas de plataforma existentes y SHALL aceptar `machineName`, limitado a las identidades activas `angel-mac` o `mac-m5`, la ruta del checkout operativo y `agents` (lista de adaptadores; vacía o ausente = autodetectar todos con `detect()`). La configuración operativa MUST rechazar la identidad histórica `old-mac`, MUST NOT requerir `serverUrl`, `machineToken`, `ADMIN_TOKEN` ni `DASHBOARD_TOKEN`, y MUST NOT persistir credenciales Git.
...
- **WHEN** el config declara `old-mac`, otra identidad, JSON malformado o tipos erróneos

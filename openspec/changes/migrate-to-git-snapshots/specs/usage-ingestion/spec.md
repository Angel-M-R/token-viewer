...
**Reason**: Las tres identidades son fijas y no necesitan registro ni tokens de máquina; solo `angel-mac` y `mac-m5` son publicadoras activas.
**Migration**: Configurar explícitamente `angel-mac` o `mac-m5` y su checkout operativo; conservar `old-mac` únicamente como histórico de snapshots.

...
Todas las consultas locales de estadísticas SHALL aceptar los filtros comunes `machine`, `agent`, `provider`, `model` y rango de fechas, validarlos y aplicarlos sobre las filas horarias cargadas. La agregación MUST ejecutarse en la capa local del dashboard sin SQL, servidor ni autenticación Bearer, y MUST conservar una experiencia interactiva con el histórico disponible de `angel-mac`, la `aon-mac` retirada y `aon-mac-m5`.

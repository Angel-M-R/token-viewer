...
- [x] 1.1 Split the closed machine registry in `packages/core` into snapshot identities (`angel-mac`, historical `aon-mac`, `aon-mac-m5`) and active publisher identities (`angel-mac`, `aon-mac-m5`), while retaining the daily snapshot, hourly usage, sanitized quota, and canonical unknown-dimension types.
- [x] 1.2 Update canonical path parsing and whole-set validation to accept all three snapshot identities, including historical `aon-mac`, while preserving machine/date agreement, UTC membership, duplicate aggregate keys, ordering, finite non-negative values, and derived totals.
...
- [x] 3.1 Restrict collector configuration to active publisher identities `angel-mac` and `aon-mac-m5`, reject historical `aon-mac` before scanning or writing, and retain operational checkout path, adapter selection, and external Git credentials; verify with focused config/CLI tests only.
...
- [x] 4.2 Update the local query repository and machine filters to expose all three valid snapshot identities, including read-only historical `aon-mac`, while preserving combined agent, provider, model, and date filtering.
...
- [x] 4.8 Update focused dashboard fixtures and tests to cover all existing views, combined filters, all three identities, historical `aon-mac` visibility, empty ranges, unknown prices, timezone conversion, invalid snapshots, and absence of login or individual data; do not require a build or full repository suite.
...
- [x] 5.1 Update publisher preflight for a dedicated clean checkout on `master`, expected private remote, and exclusive folder ownership so only `angel-mac` and `aon-mac-m5` may publish while `aon-mac` is rejected before generation or Git; whole-set snapshot validation MUST still accept its historical folder.
...
- [x] 5.4 Update focused temporary-remote integration tests for no-op runs, successful publication, concurrent disjoint commits from `angel-mac` and `aon-mac-m5`, retired-identity rejection, non-fast-forward retry, exhausted retries, network failure, conflict, and pending-commit recovery; do not run a build or full repository suite.
- [x] 5.5 Restrict the `launchd` plist installer to `angel-mac` and `aon-mac-m5`, retaining daily scheduling, explicit PATH and working directory, local logs, and no embedded credentials, and add focused checks that `aon-mac` fails before any plist is created or loaded.
- [x] 5.6 Update installation, status, log inspection, manual-run, disable, and uninstall documentation for the two active Macs and their dedicated `master` checkouts, explicitly documenting that retired `aon-mac` has no installation or publication path.
...
- [x] 6.3 Add representative sanitized snapshot fixtures for all three identities, including historical `aon-mac` and active `aon-mac-m5`, and use bounded validation timing/size checks that do not require a build or full repository suite.
...
- [x] 7.3 Update focused migration fixtures/tests so SQLite import and comparison accept historical `aon-mac`, keep its already imported aggregate snapshots immutable, do not assign legacy history to `aon-mac-m5`, and retain duplicate-row, malformed-row, privacy, and strict folder-ownership coverage without a build or full suite.
- [x] 7.4 Complete the local-source backfill for `angel-mac`, accept the already imported aggregate SQLite snapshots as the complete available history for retired `aon-mac`, require no legacy backfill for `aon-mac-m5`, and validate the resulting snapshot set with bounded commands.
...
- [x] 7.6 Validate every dashboard view and filter against historical `aon-mac`, complete `angel-mac` backfill, and representative or first-published `aon-mac-m5` data using focused dashboard and equivalence checks; record only non-sensitive discrepancies and do not require a full repository suite or build.
...
- [ ] 8.2 After `aon-mac-m5` arrives and the migration branch is integrated normally, configure its dedicated operational `master` checkout and active identity, install its daily `launchd` job, and verify one successful generation/publication in `snapshots/aon-mac-m5/`; do not install anything for retired `aon-mac`.
- [ ] 8.3 Install or confirm the dedicated operational `master` checkout and daily `launchd` job for `angel-mac`, then observe a bounded concurrent publication with `aon-mac-m5` proving successful rebase/retry behavior, no cross-folder changes, and no writes under `snapshots/aon-mac/`.
- [ ] 8.4 Verify the repository is private and obtain explicit cutover approval based on the complete three-identity snapshot set, overlap-aware equivalence report, dashboard review, successful `aon-mac-m5` publication, concurrent active-publisher evidence, and other focused evidence from completed migration tasks; final full-suite acceptance remains deferred until every planned task is complete.
...
- [ ] 8.9 Test the documented rollback drill by disabling both active `launchd` jobs and verifying the tag plus offline SQLite backup can restore the prior system without force-pushing, deleting snapshots, or treating retired `aon-mac` as a publisher.

## Final OpenSpec Verification Acceptance (Outside Planned Tasks)

After every planned task above is complete, `openspec-verifier` exclusively owns mandatory final acceptance. It MUST run full snapshot and privacy validation, the full repository test and typecheck suites, all applicable builds, and a local dashboard smoke check. Acceptance MUST confirm loading of all three snapshot identities, strict equivalence inside overlapping coverage, expected-addition classification outside legacy coverage, publication only from `angel-mac` and `aon-mac-m5`, absence of `/api/v1/*`, post-retirement integrity, and absence of any public-hosting or raw-data path. This verification is not an implementation checkbox and MUST NOT be used as a completion condition for any planned task.

...
- [x] 11.2 Recreate the operational checkout for `mac-m5` from scratch against the rewritten public repository, discarding the invalidated pre-anonymization checkout for the new M5 Mac whose identity and history are obsolete.
- [x] 11.3 After `mac-m5` arrives and the migration branch is integrated normally, configure its dedicated operational `master` checkout and active identity, resume and install its daily `launchd` job (installation stays paused until this change is implemented), and verify one successful generation/publication in `snapshots/mac-m5/`; do not install anything for retired `old-mac`.
- [x] 11.4 Install or confirm the dedicated operational `master` checkout and daily `launchd` job for `angel-mac`, then verify with real two-machine publications that each active publisher changes only its own snapshot folder and neither writes under `snapshots/old-mac/`. Focused integration coverage remains the retry-behavior evidence; by explicit user decision, a live non-fast-forward/rebase retry observation is not an acceptance requirement.
  - Completion evidence (2026-07-31): `angel-mac` has an installed and loaded daily operational job with a clean dedicated `master` checkout. Publications `7b863bd3` (`angel-mac`), `785f00a3` (`mac-m5`), `5b6dd8be` (`mac-m5`), and `cdf1a693` (`angel-mac`) all exited 0, changed only the publishing machine's snapshot folder, and made no `snapshots/old-mac/` writes. Every live publication reported `pushAttempts: 1`; no live retry is claimed.
